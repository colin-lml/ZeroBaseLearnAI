...
class TRPO:
    ...
    def line_search(self, states, actions, advantage, old_log_probs,old_action_dists, max_vec):  # 线性搜索
        ...
        temp_actor = PolicyNet(self.state_dim, self.hiddendim, self.action_dim).to(self.device)
        temp_actor.load_state_dict(self.actor.state_dict())
        ...
        for i in range(15):  # 线性搜索主循环
            ...
            torch.nn.utils.convert_parameters.vector_to_parameters(
                new_para, temp_actor.parameters())
            new_action_dists = torch.distributions.Categorical(temp_actor(states))
            kl_div = torch.mean( torch.distributions.kl.kl_divergence(old_action_dists,new_action_dists))
            new_obj = self.compute_surrogate_obj(states, actions, advantage,old_log_probs, temp_actor)
